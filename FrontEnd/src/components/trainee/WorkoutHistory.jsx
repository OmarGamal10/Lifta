...
const WorkoutHistory = ({ userId, view = true, isEditable }) => {
  ...
  return (
    <div className="w-full max-w-[95vw] mx-auto p-6 text-textColor">
      ...
      {workouts
  .filter((workout) => {
    // Filter out workouts based on the conditions
    if (!isEditable && workout.trainer_id !== decoded.user_id) {
      return false; // Exclude this workout
    }
    return true; // Include this workout
  })
  .map((workout) => (
    <Accordion
    ...
    >
      ...
      >
        <div className="flex justify-between w-full items-center">
          <h3 className="text-lg font-medium text-accent">{workout.name}</h3>
          ...
          {workout.exercises.map((exercise) => (
            ...
            >
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-accent">{exercise.musclegroup}</span>
              </div>
